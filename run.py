...
def nuevatransaccion():
    if request.method == 'GET':
        return render_template('nuevatransaccion.html')
    else:
        ...
        if datos[0] == 'EUR':
            if isfloat(cantidad) == True and float(cantidad) > 0:
                ...
                return render_template('nuevatransaccion.html', textoerror=textoerror)
...
def calculadora():
    ...
    if datos[0] == 'EUR':
        if isfloat(cantidad) == True and float(cantidad) > 0:
            ...
            return render_template('transaccion-validada.html',  preciounitario=preciounitario, tocantidad=tocantidad,cantidadamount=cantidadamount,monedafrom=monedafrom,monedato=monedato)
        else:
            textoerror = []
            textoerror.append("Valor no válido")
            return render_template('nuevatransaccion.html', textoerror=textoerror)
